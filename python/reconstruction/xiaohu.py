...
def Pre_For_Reconstruction():
    ...
    global NUM_PER_BATCH
    trainFolder = "/database/pic-data/mult-PIE_single_angle/training"
    testFolder = "/database/pic-data/mult-PIE_single_angle/test"


    reszFolder = "/home/chengcheng/debug_data/resize"
    meanImg_dir = "/home/chengcheng/debug_data/resize"
    stdImgfolder = "/database/multipie/align_multipie_45d_grey/photo_book"
    out_dir = "/database/ChengCheng/trainBatches/mult-PIE/mult-PIE_single_angle"
    #collectImgByName(trainFolder,reszFolder)
    #doGetMeanImg(reszFolder,meanImg_dir)
    #flist = labels_info(trainFolder,testFolder)
    #list.sort(flist)

    # label = np.zeros((len(flist)),dtype=np.single))
